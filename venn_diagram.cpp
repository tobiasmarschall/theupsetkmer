...
int main(int argc, char *argv[])
{
	if (argc < 5){
		...
		cout << "Usage: ./venn_classes <graph_file.gfa> <color_file.bfg_colors> <k> <threads>" << endl;
	}
	else {
	    ...
	    for (size_t i = 0; i < colors_names.size(); ++i) cout << "Color " << i << " is " << colors_names[i] << endl;
	    ...
	    for (size_t idx = 1; idx < nb_classes; ++idx){
	    	...
	    	for (size_t i = 0; i < nb_colors; ++i){
	    		...
	    		if (static_cast<bool>(idx & (0x1ULL << i))) cout << i << "-";
	    	}

	    	cout << " has " << count_classes[idx] << " k-mers" << endl;
	    }
	}
}
